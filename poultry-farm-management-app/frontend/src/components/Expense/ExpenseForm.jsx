  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(); }} style={{ marginBottom: 12 }}>
      ...
      <div style={{ marginTop: 8 }}>
        <input name="note" placeholder="Note" value={formData.note || ""} onChange={handleChange} style={{ width: "60%" }} />
        <button type="submit" style={{ marginLeft: 8 }}>{editingId ? "Update" : "Save"}</button>
      </div>
    </form>
  );
};
